...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
      title: 'Flutter Demo',
      theme: ThemeData(
        colorScheme: ColorScheme.fromSeed(seedColor: const Color.fromARGB(255, 78, 80, 93)),
        useMaterial3: true,
      ),
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  void doTheThing() async {
    ...
    var status = await Permission.microphone.status; // determine if we have microphone access
  ...
  void recording() async {
    ...
    // if microphone is not recording, then start
      ...
      record.start(
        path: tempFile,
        encoder: encoder // attempt to specify type of encoding to make it easier to read in data
      );
    }
  }
}
